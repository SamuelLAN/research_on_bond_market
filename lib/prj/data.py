...
"""
Lib for loading data in different format
"""

# config for the start date and the end date of the training set
#   the test set would start from the train_end_date
...
def load_trace_pickle(_use_cache=True):
    """
    Loading the original FINRA_TRACE_XXX.pkl data
    :return:
        [
            {
                'bond_id': 'bond_APPL',
                'offering_date': '2015-02-23',
                'report_dealer_index': '23',
                'contra_party_index': '6',
                'date': '2015-01-20',
                'volume': 12121322.0,
            },
            ...
...
def traverse_trace_pickle(fn, _dict, filter_new_bond_date='2014-06-01'):
    """
    traverse the FINRA_TRACE_XXX pickle data
    :params:
        fn: the function that will be executed during the traverse
                fn(i, val, _dict)
        _dict: to save any variables during the traverse
        filter_new_bond_date: the date for filtering new-issued bonds;
                                any bonds that were issued after this date would be filtered
    """
...
def load_dealer_2_history(_use_cache=True, filter_new_bond_date='2014-06-01'):
    """
    Load all the dealer transaction history before filtering
    :return:
        d_dealers: (dict) the key would be the dealer index,
                            the value would be its transaction history
    """
...
def load_dealer_2_dealer(_use_cache=True, filter_new_bond_date='2014-06-01', keep_dealers={}):
    """
    Load the dealer_2_dealer information as a dict
    :return:
        d_dealer_2_dealer: (dict)
                            {
                                dealer_id1: {
                                    dealer_id2: {
                                        'count': 2323, # the count of transactions between dealer id1 and id2
                                        'volume': 324542.0 # total volume of transactions between dealer id1 and id2
                                        'bonds': {
                                            'bond_APPL': 56, # the count of transactions that dealer id1
                                                             #   trade with dealer id2 for bond_APPL
                                            ...
                                        }
                                    },
                                    ...
...
def load_train_dealer_2_history():
    """
    Load the train dealer transaction history before filtering
    :return:
        train_d_dealers: (dict) the key would be the dealer index,
                                the value would be its transaction history before train_end_date
    """
    return dict(map(
        lambda x: (
            x[0],
...
def load_test_dealer_2_history():
    """
    Load the test dealer transaction history before filtering
    :return:
        test_d_dealers: (dict) the key would be the dealer index,
                                the value would be its transaction history after train_end_date
    """
    return dict(map(
        lambda x: (
            x[0],
...
def load_d_dealers_after_filtering():
    """
    Load the dealer transaction history after filtering
    :return:
        train_d_dealers: (dict) the key would be the dealer index,
                                the value would be its transaction history before train_end_date
        test_d_dealers: (dict) the key would be the dealer index,
